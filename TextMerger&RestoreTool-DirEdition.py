    ...
    for root, dirs, files in os.walk(txt_folder):
        for file in files:
            if file.lower().endswith(".txt"):
                ...
                relative_path = os.path.relpath(full_path, txt_folder)
...
def main():
        clear_terminal()
        print(Fore.YELLOW + "Welcome to Text Merger & Restore Tool - Dir Edition - v1.04\nAuthor: Nariman\n\n" + Style.RESET_ALL)
        ...
        if mode == '1':
            ...
            if os.path.isdir(txt_folder):
                count = sum(
                    1
                    for _, _, files in os.walk(txt_folder)
                    for file in files
                    if file.lower().endswith(".txt")
                )
